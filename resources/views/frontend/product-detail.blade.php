                    ...
                    <div class="tp-product-details-thumb-wrapper tp-tab d-sm-flex">
                        <nav>
                            <div class="nav nav-tabs flex-sm-column" id="productDetailsNavThumb" role="tablist">
                                <button class="nav-link active" id="nav-cover-tab" data-bs-toggle="tab" data-bs-target="#nav-cover" type="button" role="tab" aria-controls="nav-cover" aria-selected="true">
                                    <img src="{{ asset('storage/product/'.$product->cover_image) }}" alt="Cover Image">
                                </button>
                                @foreach($product->images as $key => $image)
                                    <button class="nav-link" id="nav-{{ $key + 1 }}-tab" data-bs-toggle="tab" data-bs-target="#nav-{{ $key + 1 }}" type="button" role="tab" aria-controls="nav-{{ $key + 1 }}" aria-selected="false">
                                        <img src="{{ asset('storage/products/images/'.$image) }}" alt="Product Image">
                                    </button>
                                @endforeach
                            </div>
                        </nav>
                        <div class="tab-content m-img" id="productDetailsNavContent">
                            ...
                            @foreach($product->images as $key => $image)
                                <div class="tab-pane fade" id="nav-{{ $key + 1 }}" role="tabpanel" aria-labelledby="nav-{{ $key + 1 }}-tab">
                                    <div class="tp-product-details-nav-main-thumb">
                                        <img src="{{ asset('storage/products/images/'.$image) }}" alt="Product Image">
                                    </div>
                                </div>
                                           ...
                                           <div class="tp-product-details-review-input-wrapper">
                                              <div class="tp-product-details-review-input-box">
                                                 <div class="tp-product-details-review-input">
                                                    <textarea id="msg" name="message" placeholder="Write your review here..."></textarea>
                                                 </div>
